...
class product_return
{
	...
	function product_return($db,$fnc)
	{	
	  $this->fnc = $fnc; 
	  $this->db  = $db;
	...
	function dataList($tbl,$id=null,$limit,$fiel=null,$sort,$field_sort=null,$join=null,$key_search=null,$field_search=null)
	{
		$this->db->Pagelen = $limit;
			$where.='  tb_return.id_member = tb_member.id_member and  tb_return.id_product = tb_product.id_product '; 
		if($key_search!=''){
			$where .= ' and '.$field_search.' like "%'.$key_search.'%"';
		}

		$where .= ' group by tb_return.id_rent ';
		
		$cnt	=	$this->db->countRow($tbl, $where);
		$limit  =   $this->db->countRowPage($cnt);
		return $this->db->result($tbl.$join,$where,'tb_return.'.$this->check_field_sort($fiel,$field_sort).'  '.$this->check_sort($sort).'',$limit);
	}
