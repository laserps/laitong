<?php
class product_no_return
{
	var $db;
	var $fnc;

	
	function product_no_return($db,$fnc)
	{	
	  $this->fnc = $fnc; 
	  $this->db  = $db;
	 ...
	 function dataList($tbl,$id=null,$limit,$fiel=null,$sort,$field_sort=null,$join=null,$key_search=null,$field_search=null)
	{
		$this->db->Pagelen = $limit;
			$where.='tb_rent_temp.id_product = tb_product.id_product and tb_rent_temp.id_member = tb_member.id_member    '; 
		if($key_search!=''){
			$where .= ' and '.$field_search.' like "%'.$key_search.'%"';
		}
		
		$cnt	=	$this->db->countRow($tbl.",tb_rent_temp,tb_member", $where);
		$limit  =   $this->db->countRowPage($cnt);
		return $this->db->result($tbl.",tb_rent_temp,tb_member",$where,'tb_product.'.$this->check_field_sort($fiel,$field_sort).'  '.$this->check_sort($sort).'',$limit);
	}
	...
	function dataList2($tbl,$id=null,$limit,$fiel=null,$sort,$field_sort=null,$join=null,$key_search=null,$field_search=null)
	{
		$this->db->Pagelen = $limit;
		if($key_search!=''){
			$where = ''.$tbl.'.'.$field_search.' like "%'.$key_search.'%" and status_product ="1"';
		}else{
