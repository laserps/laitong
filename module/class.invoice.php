...
class invoice
{
	...
	function Del_invoince($id,$tbl,$page,$fiel,$id_branch)
	{
		$this->db->delete(''.$tbl.'', array(''.$fiel.'' => $id));
		$this->fnc->redirect(''.$page.'');
	}
	...
	function dataList($tbl,$id=null,$limit=null,$fiel=null,$sort=null,$join=null)
	{
		...
		if($id!=''){
			...
			$data_list =  $this->db->result($tbl." group by invoice_no,id_branch ", $where);
	...
	function dataList__($tbl,$id=null,$limit=null,$fiel=null,$sort=null,$join=null)
	{
		if($limit!=""){
			$this->db->Pagelen = $limit;
		}
			...
			$data_list =  $this->db->result($tbl."", " tb_invoice.id_branch = '".$id."' group by invoice_no,id_branch  ");

			$cnt = count($data_list);
			$limit  =   $this->db->countRowPage($cnt);

		if($limit!=""){
			return $this->db->result($tbl." where tb_invoice.id_branch = '".$id."' ".$join,null,''.$tbl.'.id_invoice '.$sort.'',$limit);
		}else{
			return $this->db->result($tbl.$join);
		}
	}
