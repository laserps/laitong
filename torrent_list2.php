...
where tb_torrent.status ='1' and tb_torrent.status_tran = '0'
group by tb_torrent.id_torrent");
	...
	if($_GET['machine']!=""){
	$data_update = array("machine_plate"=>$_GET['machine']);
	$sys->db->update($tbl, $data_update,"id_torrent = '".$_GET['id']."'");
	$sys->fnc->redirect("?op=torrent&act=list&status_menu=1&status_tran_menu=0");
	}
...
<script type="text/javascript">
		function fnc_m(a,id){
			window.location.replace("?op=torrent&act=list&status_menu=1&status_tran_menu=0&machine="+a+"&id="+id+"");
		}
		</script>
...
<table width="100%" cellspacing="0" cellpadding="1" border="0" >
    ...
    <tr  bgcolor="#BBD8EC">
	  ...
	  <td width = "84" align = "center"><IMG SRC="Admin_files/true.gif" WIDTH="14" HEIGHT="14" BORDER="0" ALT="" style = "display:none;"  id = "check_true" onclick = "handler();"><IMG SRC="Admin_files/false.gif" WIDTH="14" HEIGHT="14" BORDER="0" ALT="" onclick="handler();" id = "check_false"></td>
...
?>
	 ...
	 <?}?>
	  ...
	  <td align="center" style="color:#666"><img src="../img_product/<?=$rs->pic_category;?>" border="0" /> <br><?=$rs->name_category;?></td>
	  ...
	  <select name="" onchange = "fnc_m(this.value,<?php echo $rs->id_torrent?>);">
	    <option value = "0">เลือกเครื่องยิง</option>
		<option value="1" <?php echo $fnc->selected($rs->machine_plate,1);?>>เครื่อง1</option>
		<option value="2" <?php echo $fnc->selected($rs->machine_plate,2);?>>เครื่อง2</option>
	  </select>
	  </td>
