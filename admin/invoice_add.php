...
$rs_brance = $sys->record("tb_branch"," id_branch = '".$_POST['id_branch']."' ");
...
$rs_si_wage_backmunk = $sys->record("tb_backmunk_".check_tb($rs->type)."", " id_backmunk = '".$rs->id_back_munk."' "," si_backmunk_".check_tb($rs->type)."_".$rs->size." as si, wage_backmunk_".check_tb($rs->type)."_".$rs->size." as wage ");
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

///////////////////////// เช็ค si Medicine ลงยา///////////////////////////////////////////////////////////////////////////
$rs_si_wage_medicine = $sys->record("tb_medicine_".check_tb($rs->type)."", " id_medicine = '".$rs->id_medicine."' "," si_medicine_".check_tb($rs->type)."_".$rs->size." as si, wage_medicine_".check_tb($rs->type)."_".$rs->size." as wage ");
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="th2">
				...
				<table class="th2" cellpadding="5" border="0">
					...
					<tr>
					  ...
					  <td>
						...
						&nbsp;&nbsp;<input type="radio" name="type" id = "type" value = "5" onclick = "fnc_proid_1(5);" <?php echo $fnc->checked("5",$rs->type);?>>:กรอบ 75
					...
					<tr>
					  <td width = "15%" align = "right">
						<b>ค่าแรงพิเศษ:</b></td><td><input type="text" name="special_wage" id = "special_wage" style = "width:80px;"  value = "<?php if($rs->special_wage !=""){echo $rs->special_wage;}else{?>0<?php }?>">&nbsp;&nbsp;หมายเหตุ:<input type="text" name="remark_wage" style = "width:300px;" value = "<?php echo $rs->remark_wage;?>"></td>
					</tr>
		...
		<?php if($rs->type != 4){?>
					...
					<?php if($rs->id_diamond > 13){
						$rs_diamond_wage_first = $sys->record("tb_diamond_".part_tb($rs->type).""," id_diamond = '".$rs->id_diamond."' "," wage_diamond_".part_tb($rs->type)."_".$rs->size." as wage_first_diamond ");

						//echo $rs_diamond_wage_first->wage_first_diamond;
						$rs_content_product = $sys->record("tb_content_product"," id_product = '".$rs->id_product."' "," wage_fung ");

						$wage_diamond_first_line = ($rs_content_product->wage_fung * $rs->total_diamond);
						$wage_diamond = ($rs->total_diamond*$rs->karat*$rs->per_karat)+$wage_diamond_first_line+$rs_diamond_wage_first->wage_first_diamond;
						//echo $wage_diamond;
					?>
					...
					<tr>
					  <td width = "15%" align = "right">
						...
						&nbsp;X&nbsp;<input type="text" name="wage_fung" id = "wage_fung"  style = "width:50px;" value = "<?php echo $rs_content_product->wage_fung;?>" onclick = "document.getElementById('wage_fung').value = '';" onblur = "fnc_wage_fung();">&nbsp;
						= <span style = "color:#ff0000;" id = "diamond_line"><?php echo $rs->total_diamond*$rs_content_product->wage_fung;?></span>
						<input type="hidden" id="total_wage_first_line" value = "<?php echo $rs->total_diamond*$rs_content_product->wage_fung;?>"><br>
					...
					<?php
						...
						$row_special_sub_category = $sys->result("tb_special_sub_category_".part_tb($rs->type).",tb_special",' tb_special_sub_category_'.part_tb($rs->type).'.id_sub_category = "'.$rs->id_sub_category.'" and tb_special_sub_category_'.part_tb($rs->type).'.id_special = tb_special.id_special   ');

						$row_speciao_product = $sys->result(" tb_special_product,tb_special_".part_tb($rs->type)." "," tb_special_product.id_product = '".$rs->id_product."' and tb_special_product.id_special = tb_special_".part_tb($rs->type).".id_special  "," tb_special_product.id_product asc",null," wage_special_".part_tb($rs->type)."_".$rs->size." as wage_special ");

						if(!empty($row_speciao_product )){
						foreach($row_speciao_product as $rs_sp){
							$wage_special = $wage_special+$rs_sp->wage_special."<br>";
						}
						}

						/*foreach($row_special_sub_category as $rs_special){
			...
			<td align="left" bgcolor="#ccffcc" valign = "top">
				...
				<?php 
					if($_POST['gold_text_re'] != ""){
						$gold__ = $_POST['gold_text_re'];
					}
				?>

				<tr>
					<td align = "center">
					<div style="border: 3px coral solid; width: 120px;height:100px;"><h4 >ราคาทอง</h4><h2 id = "gold_text">
					<input type="text" id="gold_text_re" value = "<?php echo $gold__;?>" style = "font-size: 20px; width: 70px; text-align: center; background-color: #b9b900;" onkeydown = "fnc_calculate_gold();">&nbsp;<input type="button" value="Load" onclick="re_gold();">
					</h2><input type="hidden" id ="gold" value = "<?php echo $gold__;?>">
				...
				<tr>
					<td align = "center">
					<div style="border: 3px coral solid; width: 120px;height:100px;"><h4>เปอร์เซ็น</h4><input type="text" id="percent_" value = "<?php if($_POST['percent_'] != ""){ echo $_POST['percent_'];}else{ echo $rs_percent->percent_;}?>" style = "font-size: 22px; width:60px; text-align: center;">%<input type="button" value="Load" onclick="re_percent();">
				...
				<?php 
					function check_calculate_type($status_calculate,$price){
							if($status_calculate == "2"){
								$total_price =	ROUND($price/50)*50; 
								return $total_price;
							}else{
								return $price;
							}
				...
				<tr>
					...
					<h2 id="total_wage_text"><?php
					$total_wage___ = ceil($rs_wage_first->wage+$wage_special+$wage_diamond+$rs->special_wage+$rs_si_wage_backmunk->wage+$rs_si_wage_medicine->wage);

					echo $total_wage___;?></h2><input type="hidden" name="total_wage" id="total_wage"  value = "<?php echo $total_wage___;?>">
				...
				<tr>
					...
					if($_POST['percent_'] != ""){
						$price_ =  
					( (($gold__ * 0.0656 * $_POST['percent_'])/100) * ($rs->weight + $rs->si_product) )   + ($total_wage___);

					}else{
						$price_ = ( (($gold__ * 0.0656 * $rs_percent->percent_)/100) * ($rs->weight + $rs->si_product) )   + ($total_wage___);
					}
					$old_price = $price_;

					$price_ = check_calculate_type($rs_brance->status_calculate,$price_);
					/*$price_ =  
					( (($gold__ * 0.0656 * $rs_percent->percent_)/100) * ($rs->weight + $rs->si_product) )   + ($rs->total_wage);*/
				 ?>
					<div style="border: 3px #ff0000 solid; width: 120px;height:120px;"><h4 >ราคา</h4><h2 id="price_text">
					<?php if($rs_brance->status_calculate == "2"){?><span style = "color: #ff0000; font-size: 14px;">(<?php echo (int)$old_price;?>)</span></br><?php }?>&nbsp;
